...
in
{
  home.packages = with pkgs; [
    (writeShellApplication {
      ...
      text = ''
        ...
        #        REPO_NAME=$(basename "${config.source.url}")
                ...
                move2config(){
                  ...
                  if rsync -P --stats "$TEMP_DIR/" "$HM_DIR/" 2>/dev/null; then
                    rm -rf "$TEMP_DIR"
                    return 0
                  fi
                  ...
                  # Fallback avec cp
                  info "rsync indisponible, utilisation de cp..."
                  cp -r "$TEMP_DIR/" "$HM_DIR" && rm -rf "$TEMP_DIR" && return 0
                ...
                build_conf(){
                  ...
                  if home-manager switch --flake "$HM_DIR" -b "backup" 2>&1 | nom; then
                    return 0
                  fi

                  fail "Échec du rebuild. Reessayer manuellement avec: home-manager switch --flake $HM_DIR"
                }
                # -- main -- #
                main() {
                  header
                  ...
                  for cmd in git gum dix nom home-manager; do
                    command -v "$cmd" &>/dev/null || info "Manquant: $cmd"
                  done
                  ...
                  fetch_update
                  success "Fetched depuis '${config.source.url}'"
      ...
      meta = {
        description = "Mise à jour automatique de la configuration - Binary Club";
        version = "1.0.0";
